...
from wavernn_vocoder.utils.display import stream, simple_table
from wavernn_vocoder.utils.dataset import get_vocoder_datasets
from wavernn_vocoder.utils.distribution import discretized_mix_logistic_loss
from wavernn_vocoder.utils import hparams as hp
from wavernn_vocoder.models.fatchord_version import WaveRNN
from wavernn_vocoder.gen_wavernn import gen_testset
from wavernn_vocoder.utils.paths import Paths
import argparse
from wavernn_vocoder.utils import data_parallel_workaround
from wavernn_vocoder.utils.checkpoints import save_checkpoint, restore_checkpoint
import os
...
def main():
    ...
    parser.add_argument('--gta', '-g', default= True, action='store_true', help='train wavernn on GTA features')
    parser.add_argument('--force_cpu', '-c', action='store_true', help='Forces CPU-only training, even when in CUDA capable environment')
    parser.add_argument('--hp_file', metavar='FILE', default='wavernn_vocoder/hparams.py', help='The file to use for the hyperparameters')
    ...
    train_set, test_set = get_vocoder_datasets(wavernn_data, batch_size, train_gta)
