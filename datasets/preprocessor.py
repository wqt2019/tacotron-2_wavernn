...
def build_from_path_mydata(hparams, input_dirs, mel_dir, linear_dir, wav_dir, n_jobs=12, tqdm=lambda x: x):
        ...
        for input_dir in input_dirs:
            with open(os.path.join(input_dir, 'ProsodyLabeling', 'biaobei_transcript_phone.txt'), encoding='utf-8') as f:
                for line in f:
                    ...
                    basename = parts[0]
                    wav_path = os.path.join(input_dir, 'Wave', '001', '{}.wav'.format(basename))
                    text = parts[-1]
