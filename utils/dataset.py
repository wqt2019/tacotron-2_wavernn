...
class ljdataset(Dataset):
	def __init__(self, fdir):
		metadata_fpath = fdir + 'train.txt'
		with open(metadata_fpath, "r") as metadata_file:
			self.f_list = [[line.split("|")[-1],os.path.join(fdir,'mels',line.split("|")[1])] for line in metadata_file]
			random.shuffle(self.f_list)

	def __getitem__(self, index):
		text, mel = get_mel_text_pair(*self.f_list[index])
		return text, mel
