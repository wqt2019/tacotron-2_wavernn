...
class VocoderDataset(Dataset):
    ...
    def __init__(self, path, dataset_ids):
        ...
        print("Found %d samples" % len(self.metadata))

    def __getitem__(self, index):
        mel_path, wav_path = self.metadata[index]
        hop_length = hp.hop_length
        voc_mode = hp.voc_mode
        ...
        wav = np.load(wav_path).T
        wav = np.clip(wav, -1, 1)
        # Fix for missing padding   # TODO: settle on whether this is any useful
        r_pad = (len(wav) // hop_length + 1) * hop_length - len(wav)
        wav = np.pad(wav, (0, r_pad), mode='constant')
        assert len(wav) >= mel.shape[1] * hop_length
        wav = wav[:mel.shape[1] * hop_length]
        assert len(wav) % hop_length == 0
        # Quantize the wav
...
def get_vocoder_datasets(path: Path, batch_size, train_gta):
    ...
    gta_fpaths = [(gta_path + fname) for fname in gta_fnames]
    wav_fnames = [x[0] for x in metadata if int(x[4])]
    wav_fpaths = [(audio_path + fname) for fname in wav_fnames]
    samples_fpaths = list(zip(gta_fpaths, wav_fpaths))
    print("Found %d samples" % len(samples_fpaths))

    test_ids = samples_fpaths[-hp.voc_test_samples:]
    train_ids = samples_fpaths[:-hp.voc_test_samples]

    random.seed(1234)
    # random.shuffle(dataset_ids)

    train_dataset = VocoderDataset(path, train_ids)
    test_dataset = VocoderDataset(path, test_ids)
